...
$c->iniCheck('Ensuring that detect_unicode is disabled', 'detect_unicode', false, true,
        'Enabling detect_unicode may cause errors when using phar files. See https://bugs.php.net/bug.php?id=42396');
...
if (extension_loaded('suhosin')) {
    $c->addRequire(
            ...
            sprintf('suhosin.executor.include.whitelist must be configured to include "phar" in %s so that the phar file works correctly',
                    php_ini_loaded_file())
    );
}
...
if (extension_loaded('xdebug')) {
    ...
    $c->iniCheck('Ensuring that Xdebug\'s infinite recursion detection does not erroneously cause a fatal error',
            'xdebug.max_nesting_level', 0, false);
}
