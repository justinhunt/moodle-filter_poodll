...
class Lexer {
    ...
    public function tokenize($input) {
        ...
        while (false !== ($current = current($chars))) {
            ...
            } else if ($state === self::STATE_IDENTIFIER) {
            ...
            } else if ($state === self::STATE_WHITESPACE) {

                // Skip whitespace
                next($chars);

            } else if ($state === self::STATE_LBRACKET) {
            ...
            } else if ($state === self::STATE_STRING_LITERAL) {
            ...
            } else if ($state === self::STATE_PIPE) {

                // Consume pipe and OR
                $tokens[] = $this->matchOr($chars, '|', '|', self::T_OR, self::T_PIPE);

            } else if ($state == self::STATE_JSON_LITERAL) {
            ...
            } else if ($state == self::STATE_NUMBER) {
            ...
            } else if ($state === self::STATE_QUOTED_STRING) {
